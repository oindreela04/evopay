...
export function AppShell({ children, path, onNavigate }: { children: ReactNode; path: string; onNavigate: (path: string) => void }) {
 ...
 return <div className="app-shell"><aside className="sidebar"><div className="brand"><div className="brand-symbol"><Blocks size={17} /></div><span>EvoPay <b>AI</b></span></div><div className="workspace-label">Security workspace <span className="workspace-status" /></div><nav>{nav.map((item) => { const Icon = item.icon; return <button key={item.path} className={`nav-item ${path === item.path ? 'active' : ''} ${item.group ? 'group-start' : ''}`} onClick={() => onNavigate(item.path)}><Icon size={17} /><span>{item.label}</span>{path === item.path && <i />}</button> })}</nav><div className="sidebar-bottom"><button className="run-demo-button" onClick={startDemo}><Zap size={15} /> Run Demo</button><button className="nav-item" onClick={() => onNavigate('/settings')}><Settings size={17} /><span>Settings</span></button><div className="system-card"><div className="system-top"><span>System status</span><span className="system-live">99.99%</span></div><div className="system-bar"><i /></div><small><span className="pulse-dot" /> All services operational</small></div><div className="sidebar-footer"><span><CircleGauge size={14} /> v0.1.0-beta</span><span><GitBranch size={13} /> main</span></div></div></aside><section className="main-pane">{children}</section>{demoStep !== null && <DemoOverlay step={demoStep} restart={restartDemo} stop={stopDemo} />}</div>
}
